...
class MFCCRawDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return (
            feat,
            self.labels_plate[idx],
            self.labels_electrode[idx],
            self.labels_current[idx],
        )
...
def main():
    ...
    cache_path = (
        duration_dir / "mfcc_cache" / f"xvector_features_overlap_{args.overlap}.pt"
    )
    models_dir = (
        duration_dir
        / "modelos"
    ...
    avg_time_per_epoch = (
        sum(f.get("avg_epoch_time", 0) for f in fold_results) / len(fold_results)
        if fold_results
        else 0
    )
    ...
    print("=" * 60)
    print(
        f"  Tiempo total: {results['timing']['total_seconds']:.1f}s ({results['timing']['total_minutes']:.1f} min)"
    )
    print(
        f"  Tiempo CV (suma folds): {results['timing']['cv_seconds']:.1f}s ({results['timing']['cv_minutes']:.1f} min)"
    )
    print(
        f"  Tiempo promedio por fold: {results['timing']['avg_per_fold_seconds']:.1f}s"
    )
