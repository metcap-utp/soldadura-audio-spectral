...
def load_models(duration, overlap, k_folds, model_type, device):
    ...
    models_dir = Path(
        f"{duration:02d}seg/modelos/{model_type}/k{k_folds:02d}_overlap_{overlap}"
    )
...
def load_cached_data(duration_dir, model_type, overlap):
    ...
    cache_path = (
        duration_dir
        / "mfcc_cache"
        / cache_names.get(model_type, f"features_overlap_{overlap}.pt")
    )
...
def predict_ensemble(models, features, model_type, device):
    ...
    return (
        np.array(all_preds_plate),
        np.array(all_preds_electrode),
        np.array(all_preds_current),
    )
...
def evaluate_model(
    ...
    for i in range(n_samples):
        ...
        hamming_correct += (
            int(plate_correct) + int(electrode_correct) + int(current_correct)
        )
...
def save_confusion_matrix_plot(
    confusion_matrices, output_dir, model_type, label_encoders
):
...
def run_inference(duration, overlap, k_folds, model_type, device):
    ...
    print(
        f"  Plate: Acc={metrics['plate']['accuracy']:.4f}, F1={metrics['plate']['f1_macro']:.4f}"
    )
    print(
        f"  Electrode: Acc={metrics['electrode']['accuracy']:.4f}, F1={metrics['electrode']['f1_macro']:.4f}"
    )
    print(
        f"  Current: Acc={metrics['current']['accuracy']:.4f}, F1={metrics['current']['f1_macro']:.4f}"
    )
    ...
    save_confusion_matrix_plot(
        confusion_matrices, metricas_dir, model_type, label_encoders
    )
...
def main():
    ...
    model_types = (
        ["xvector", "ecapa", "feedforward"] if args.model == "all" else [args.model]
    )
    ...
    for model_type in model_types:
        result = run_inference(
            args.duration, args.overlap, args.k_folds, model_type, device
        )
        if result:
